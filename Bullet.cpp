...
//===NEW===
/*
Bullet::Bullet(std::string img, float speed, float damage, Engine::Point position, float rotation, Turret* parent) :
	Sprite(img, position.x, position.y), speed(speed), damage(damage), parent(parent) {
	Engine::Point dir (position.x, position.y);
	Velocity = (dir);
	Rotation = rotation;
	CollisionRadius = 4;
}*/

void Bullet::Update(float deltaTime) { //override update and use for orbitting bullet.
