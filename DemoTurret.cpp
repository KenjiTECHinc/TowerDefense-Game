...
const int DemoTurret::Price = 0;
DemoTurret::DemoTurret(float x, float y) :
    // TODO 3 (1/5): You can imitate the 2 files: 'PlugGunTurret.hpp', 'PlugGunTurret.cpp' to create a new turret.
    Turret("play/tower-base.png", "play/turret-7.png", x, y, 800, Price, 1) {
    // Move center downward, since we the turret head is slightly biased upward
    Anchor.y += 8.0f / GetBitmapHeight();
}
void DemoTurret::CreateBullet() {
    ...
    getPlayScene()->BulletGroup->AddNewObject(new DemoBullet(Position + normalized * 36, diff, rotation, this));
